...
public class AboutBookActivity extends AppCompatActivity {
    ...
    void setDataToViews(){
        ...
        intro_tv_info.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                intro_tv_info.setText(intro);
            }
        });
    ...
    void getFromDB(){
        ...
        DocumentReference docRef = db.collection("books").document(book_name+"_"+author);
  ...
  quote_tv.setText("“ "+task.getResult().getDocuments().get((int)(Math.random()*task.getResult().size())).get("text")+" ”");
    ...
    boolean deleteBookFromLocal() {
        ...
        if (!sharedPref.getString(book_name+"_"+author+"_locator","BLANK").equals("BLANK"))
        sharedPref.edit().remove(book_name+"_"+author+"_locator").apply();
...
else toastMsg(getString(R.string.download_in_prog));
        ...
        if (sharedPref.getString(book_name+"_"+author+"_locator","BLANK").equals("BLANK"))
            FolioReader.get()
                    .setReadLocatorListener(readLocator -> {
                        SharedPreferences.Editor editor = sharedPref.edit();
                        editor.putString(book_name+"_"+author+"_locator",readLocator.toJson());
                        editor.apply();
                    })
                    .setConfig(config, true)
                    .openBook(getFilesDir()+file_loc.substring(file_loc.lastIndexOf("/")));
        else
            FolioReader.get()
                .setReadLocatorListener(readLocator -> {
                    SharedPreferences.Editor editor = sharedPref.edit();
            ...
            .setReadLocator(ReadLocator.fromJson(sharedPref.getString(book_name+"_"+author+"_locator","BLANK")))
                .setConfig(config, true)
                .openBook(getFilesDir()+file_loc.substring(file_loc.lastIndexOf("/")));
